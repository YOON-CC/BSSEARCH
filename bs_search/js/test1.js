...
// ready=4인 경우 현재 응답이 완전히 도착 후 처리, 프로퍼티값이 변경될 때 마다 호출 
            ...
            for (i = 0; i < member1.length; i++) {  // 58개이다. 즉, 배열에는 57까지`
                ...
                if (condition1[i].firstChild.data == "기장군"){
                    if (i != 0) { members1 += "<br>"; }
                    if (i != 0) { members1 += "<br>"; }
                    members1 += member1[i].firstChild.data;   
                    members1 += "<br>▶"; 
                    members1 += where1[i].firstChild.data;  
                // firstchild 속성은 선택한 요소의 첫번째 자식 노드로 변환하는데
                // 선택한 노드에 자식이 없으면 NULL 으로 반환
                }  
            }
              // 기장군의 학교 명와 주소 데이터를 list1에 저장 
            document.getElementById("list1").innerHTML = members1; // 기장군의 초등학교★
        }
    };
    //기장군 중학교
     // onreadystatechange는 서버에서 응답이 도착하면 학교의 xml 데이터를 불러오는 함수를 호출
    req2.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (i = 0; i < member2.length; i++) {  // 58개이다. 즉, 배열에는 57까지`
                 //기장군의 데이터 받아오기 
                 // 리스트에 학교명 그밑에줄에 ▶ 도로명주소 으로 출력
                if (condition2[i].firstChild.data == "기장군"){
                    if (i != 0) { members1 += "<br>"; }
                    if (i != 0) { members1 += "<br>"; }
                    members1 += member2[i].firstChild.data;   
                    members1 += "<br>▶"; 
                    members1 += where2[i].firstChild.data;  
                }
                // firstchild 속성은 선택한 요소의 첫번째 자식 노드로 변환하는데
                // 선택한 노드에 자식이 없으면 NULL 으로 반환 
            }
            // 기장군의 학교 명와 주소 데이터를 list1에 저장 
            document.getElementById("list1").innerHTML = members1; //기장군 중학교★
        }
    };
    //기장군 고등학교
      // onreadystatechange는 서버에서 응답이 도착하면 학교의 xml 데이터를 불러오는 함수를 호출
    req3.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (i = 0; i < member3.length; i++) {  // 58개이다. 즉, 배열에는 57까지`
                //기장군의 데이터 받아오기 
                // 리스트에 학교명 그밑에줄에 ▶ 도로명주소 으로 출력
                if (condition3[i].firstChild.data == "기장군"){
                    if (i != 0) { members1 += "<br>"; }
                    if (i != 0) { members1 += "<br>"; }
                    members1 += member3[i].firstChild.data;   
                    members1 += "<br>▶";
                    members1 += where3[i].firstChild.data;  
            ...
            // 기장군의 학교 명와 주소 데이터를 list1에 저장 
            document.getElementById("list1").innerHTML = members1; // 기장군 교등학교★
        }
    };
}
