    ...
    req1.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (i = 0; i < member1.length; i++) {  // 58개이다. 즉, 배열에는 57까지`
                ...
                if (condition1[i].firstChild.data == "금정구"){
                    if (i != 0) { members2 += "<br>"; }
                    if (i != 0) { members2 += "<br>"; }
                    members2 += member1[i].firstChild.data;   
                    members2 += "<br>▶"; 
                    members2 += where1[i].firstChild.data;  
                        // firstchild 속성은 선택한 요소의 첫번째 자식 노드로 변환하는데
                // 선택한 노드에 자식이 없으면 NULL 으로 반환
                }  
            }
            // 금정구의 학교 명와 주소 데이터를 list2에 저장 
            document.getElementById("list2").innerHTML = members2; //금정구 초등학교★
        }
    };
    //금정구 중학교
    req2.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (i = 0; i < member2.length; i++) {  // 58개이다. 즉, 배열에는 57까지`
                 //금정구의 데이터 받아오기 
                 // 리스트에 학교명 그밑에줄에 ▶ 도로명주소 으로 출력
                if (condition2[i].firstChild.data == "금정구"){
                    if (i != 0) { members2 += "<br>"; }
                    if (i != 0) { members2 += "<br>"; }
                    members2 += member2[i].firstChild.data;   
                    members2 += "<br>▶"; 
                    members2 += where2[i].firstChild.data;  
                }
                 // firstchild 속성은 선택한 요소의 첫번째 자식 노드로 변환하는데
                // 선택한 노드에 자식이 없으면 NULL 으로 반  
            } 
            // 금정구의 학교 명와 주소 데이터를 list2에 저장 
            document.getElementById("list2").innerHTML = members2; // 금정구 중학교★
        }
    };
    //금정구 고등학교
      // onreadystatechange는 서버에서 응답이 도착하면 학교의 xml 데이터를 불러오는 함수를 호출
    req3.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            for (i = 0; i < member3.length; i++) {  // 58개이다. 즉, 배열에는 57까지`
                //금정구의 데이터 받아오기 
                // 리스트에 학교명 그밑에줄에 ▶ 도로명주소 으로 출력
                if (condition3[i].firstChild.data == "금정구"){
                    if (i != 0) { members2 += "<br>"; }
                    if (i != 0) { members2 += "<br>"; }
                    members2 += member3[i].firstChild.data;   
                    members2 += "<br>▶";
                    members2 += where3[i].firstChild.data;  
            ...
            // 금정구의 학교 명와 주소 데이터를 list2에 저장 
            document.getElementById("list2").innerHTML = members2; //금정구 고등학교★
        }
    };
}
